...
export function isStealthMode(): boolean {
  return localStorage.getItem("echat_story_stealth") === "true";
}
...
export function archiveStory(story: Story): void {
  try {
    const existing: Story[] = JSON.parse(localStorage.getItem("echat_story_archive") || "[]");
    if (!existing.find((s) => s.id === story.id)) {
      existing.unshift(story);
      localStorage.setItem("echat_story_archive", JSON.stringify(existing.slice(0, 50)));
    }
  } catch {}
}

export function getArchivedStories(userId: string): Story[] {
  try {
    const all: Story[] = JSON.parse(localStorage.getItem("echat_story_archive") || "[]");
